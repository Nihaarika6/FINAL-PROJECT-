...
public class OrderDetailRestTest {
	...
	assertEquals(OrderStatus.REJECTED, stat);
    ...
    @Test
	public void testVendorHistoryById200() throws AssertionError, URISyntaxException {
		given().accept(ContentType.JSON).when()
		.get(CommonUtil.getURI("/api/orderdetail/historyven/3")).then().assertThat().statusCode(200);
    }
    
     @Test
	public void testVendorPendingHistoryById200() throws AssertionError, URISyntaxException {
		given().accept(ContentType.JSON).when()
		.get(CommonUtil.getURI("/api/orderdetail/pendingven/1")).then().assertThat().statusCode(200);
    }
    @Test
	public void testCustomerPendingHistoryById200() throws AssertionError, URISyntaxException {
		given().accept(ContentType.JSON).when()
		.get(CommonUtil.getURI("/api/orderdetail/pendingcus/1")).then().assertThat().statusCode(200);
	}
